...
window.checkValidIndexJSON = function (csrfToken) {
  $.ajax({
    type: 'POST',
    url: `${ME_SETTINGS.baseHref}checkValid`,
    data: {
...
function renderProp(input) {
  ...
  if (
    ...
    return `<img src="${htmlEntities.encode(input)}" style="max-height:100%; max-width:100%; "/>`;
  }
  ...
  if (
    ...
    return `<audio controls style="width:45px;" src="${htmlEntities.encode(input)}">Your browser does not support the audio element.</audio>`;
  }
  ...
  if (
    ...
    return `<video controls><source type="${htmlEntities.encode(videoFormat)}" src="${htmlEntities.encode(input)}"/>
  ...
  return htmlEntities.encode(input.toString());
}
...
$(() => {
  ...
  $tableWrapper.scroll(function () {
    ...
    document.querySelector('#fadeToWhiteID').style.opacity = opacity > 0.6 ? 0.6  : opacity;
  });
