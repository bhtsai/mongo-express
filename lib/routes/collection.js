...
const routes = function (config) {
  ...
  exp._getAggregatePipeline = function (pipeline, queryOptions, facet) {
  ...
  exp._getItemsAndCount = async function (req, queryOptions) {
    ...
    if (req.query.runAggregate === 'on' && query.constructor.name === 'Array') {
      if (query.length > 0) {
        const queryAggregate = exp._getAggregatePipeline(query, queryOptions, true);
  ...
  exp.exportCsv = async function (req, res) {
    try {
      ...
      let queryResult = [];
      if (req.query.runAggregate === 'true' && query.constructor.name === 'Array') {
        if (query.length > 0) {
          const queryAggregate = exp._getAggregatePipeline(query, queryOptions, false);
          queryResult = await req.collection.aggregate(queryAggregate, { allowDiskUse: config.mongodb.allowDiskUse }).toArray();
        }
      } else {
        queryResult = await req.collection.find(query, { ...queryOptions, allowDiskUse: config.mongodb.allowDiskUse }).toArray();
      }
